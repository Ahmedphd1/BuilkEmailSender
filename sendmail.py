...
def slice_emails(email_list, target_email):
    """
    Slices the email list starting from the target_email if found.

    Args:
        email_list (list): The list of email addresses.
        target_email (str): The email address to slice from.

    Returns:
        list: The sliced list or the whole list if target_email is not found.
    """
    if target_email in email_list:
        index = email_list.index(target_email)
        return email_list[index:]  # Return slice from target_email to end
    return email_list  # Return the whole list if not found
